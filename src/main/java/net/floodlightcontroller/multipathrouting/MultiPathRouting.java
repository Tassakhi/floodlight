...
public class MultiPathRouting implements IFloodlightModule, ITopologyListener,
		...
		for (LDUpdate update : linkUpdates) {
			if (update.getOperation().equals(
				...
				LinkWithCost srcLink = new LinkWithCost(update.getSrc()
						.getLong(), update.getSrcPort().getShortPortNumber(),
						update.getDst().getLong(), update.getDstPort()
								.getShortPortNumber(), 1);
	...
	public Route buildFlowRoute(FlowId fid) {
		...
		Route result = null;
		try {
			routes = pathcache.get(new RouteId(DatapathId.of(srcDpid),
					DatapathId.of(dstDpid)));
		} catch (Exception e) {
		...
		result = new Route(new RouteId(DatapathId.of(srcDpid),
				DatapathId.of(dstDpid)), nptList);
		return result;
	}
	...
	public void generateMultiPath(MultiRoute routes, Long srcDpid,
			Long dstDpid, Long current,
			HashMap<Long, HashSet<LinkWithCost>> previous,
			LinkedList<NodePortTuple> switchPorts) {
		if (pathCount >= ROUTE_LIMITATION)
			return;
		if (current == srcDpid) {
			pathCount++;
			Route result = new Route(new RouteId(DatapathId.of(srcDpid),
					DatapathId.of(dstDpid)), new LinkedList<NodePortTuple>(
					switchPorts));
		...
		for (LinkWithCost link : links) {
			NodePortTuple npt = new NodePortTuple(DatapathId.of(link
					.getDstDpid()), OFPort.of(link.getDstPort()));
			NodePortTuple npt2 = new NodePortTuple(DatapathId.of(link
					.getSrcDpid()), OFPort.of(link.getSrcPort()));
	...
	public ArrayList<Route> getRoutes(DatapathId srcDpid, DatapathId dstDpid) {
		...
		MultiRoute myMR = new MultiRoute();
		ArrayList<Route> result = null;
		try {
			myMR = pathcache.get(id);
			result = myMR.getRoutes();
		} catch (Exception e) {
			logger.error("error {}", e.toString());
		}
